...
class OTP {
  ...
  /* ＊
     ...
     * @return {OTP}
     */
  generate_otp(input, withCounterPadding = true) {
    // generate HMAC object with SHA-1 digest
    const hmacObj = new jsSHA(this.digest, 'BYTES');
    // set hmac token
    const secretByteString = Util.byte_secret(this.secret);
    hmacObj.setHMACKey(secretByteString, 'BYTES');
    // hamc encode the input param
    let inputByteString = Util.int_to_bytestring(input);
    if (withCounterPadding) {
      inputByteString = Util.rightPadByteString(inputByteString, secretByteString.length);
    }
    hmacObj.update(inputByteString);
    ...
    const charCode = hmac_a[hmac_a.length - 1].charCodeAt();
    const offset = (charCode == 0) ? 0x00 : charCode & 0xf;
