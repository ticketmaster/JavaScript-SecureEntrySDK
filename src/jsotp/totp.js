...
class TOTP extends OTP {
  ...
  /* ＊
     ...
     * ```
     */
  now(time, withCounterPadding = true) {
    // get now time string
    const now = Util.timecode(time ? new Date(time) : new Date(), this.interval);

    // generate the one-time password
    const digit = super.generate_otp(now, withCounterPadding);
    return digit;
  }
